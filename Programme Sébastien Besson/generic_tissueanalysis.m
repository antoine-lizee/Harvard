...
if mergetissue % Remove youngest edges 
    ...
    vertices_list = zeros(N_edgestomerge,2); % Initiate the vertices to suppress
    for i=1:N_edgestomerge
        ne = find(tissue.e(:,4)==2,1); % Find the indice of the next recent edge
        vertices_list(i,:) = tissue.e(ne,1:2); % Get the list of couple of vertices index for the edge we are supressing
        tissue = merge(tissue,neighbors(tissue,ne)); % Merge the two neighbor cells
    end
...
if simplify
    ...
    % I think the following part is useless with regards to the
    % vertices_list which give us the result
    old_connectivity = arrayfun(@(x) sum(sum(originaltissue.e(:,1:2)==x)),1:length(originaltissue.v)); %Compute and stock in i-indexed array the number of vertices index equals to i in the edge array of originaltissue.
    new_connectivity = arrayfun(@(x) sum(sum(tissue.e(:,1:2)==x)),1:length(tissue.v));% Same with mergedtissue
    nv = find(new_connectivity==2 & new_connectivity~=old_connectivity); % Find the vertices to be removed by locating the indexes of vertices used 2 times in the mergedtissue but used more in the originaltissue

    bar = waitbar(0,'Removing vertices');
    for i=nv(end:-1:1) % i indice of the vertex to be removed
