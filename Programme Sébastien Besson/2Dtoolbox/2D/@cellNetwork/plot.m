...
% the plotted edges and vertices.
% * varargin - in addition to classical lineseries properties, varargin can
% include an option name 'num' which will draw the index of the vertices
% (debug)
%
%% Outputs
...
num=0; %For thr num option which enables one to print the index of the vertices
%Detect this option
for i=1:length(varargin)
    if strcmp(varargin{i},'num');
        num=1;
        varargin(i)=[];
        break
    end
...
for i = 1:size(N.v,1)
    ...
    if num, h.text(i)=text(N.v(i,1), N.v(i,2), [' ', num2str(i,'% u')]); end
end
