...
gamma=0:0.1:360;
dl_2=zeros(length(gamma),numel(n)+1);
...
for ii=n
    ...
    while k1==0 && j<=length(gamma)
        try
            [V, beta_final, ~]=findfixedpoint(ii+0.2,gamma(j),a);
            j=j+1;
            cell=getcellfromsol(V,beta_final);
            V=[cell.v(3,:)-cell.v(2,:);cell.v(ii+3,:)-cell.v(ii+2,:)];
            A=cell.e([2,ii+2],3);
            dl_2(j,ii)=sum(sqrt(sum(V.^2,2)).*A./sin(A));
        catch err
...
figure(2);
hold all;
plot(gamma,dl_2(:,n),'Linewidth',1.5);
%L_2=[num2str(n','%d edges Type 2')];
%legend(L_2)
